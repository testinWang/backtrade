...
# 多进程处理数据
def multi_pool_deal(func, data, cores=5, slice_size=10000):
...
# 多进程处理数据返回多个进程的结果的列表
def multi_pool_deal_list(func, data, cores=5, slice_size=10000):
...
def multi_pool_deal_partition_list(func, data_list, cores=5):
...
# 多进程处理数据并返回所有数据集合
def multi_pool_deal_partition(func, datas, cores=5, slice_size =None,args=None):
...
# 多进程处理数据并返回所有数据集合
def multi_pool_deal_partition_flatten(func, datas, cores=5, slice_size=None, args=None):
    data_length = len(datas)
    slice = math.floor(data_length / slice_size)
    param = []
    for i in range(slice + 1):
